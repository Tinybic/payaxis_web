...
import { projectpayment_new, receivable_list } from 'src/app/core/gql/receivables';
...
export class OrderListComponent extends Base {
  ...
  getOrders(){
    if(this.localStorage.getItem('idcompany')){
      let serviceName = this.orderType == 'Sent' ? projectorder_list : receivable_list;
  ...
  payBill(item){
    if(item.remainingAmount > 0){
      ...
      this.confirmModalRef.result.then((result) => {
        this.vendor = {
          id: item.idVendor,
          vendorName: item.vendorName
        };
