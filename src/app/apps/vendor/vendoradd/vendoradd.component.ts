...
export class VendoraddComponent {
  ...
  getVendorInfo(){
    if(this.idvendor != 0){
      this.apolloService.query(vendor_info, {id: this.idvendor}).then((res) => {
  ...
  getCostCodeList(){
    ...
    if(this.vendor.idCompany != 0){
      this.apolloService.query(categorycostcode_list, {idCompany: this.vendor.idCompany}).then((res) => {
  ...
  fileDelete(){
    ...
    if(item.id){
      this.apolloService.mutate(vendor_file_delete, {
        idCompany: this.vendor.idCompany,
  ...
  vendor_archive(){
    if(this.idvendor > 0 && this.revision > 0){
      this.apolloService.mutate(vendor_archive, {
        idCompany: this.vendor.idCompany,
        id: this.idvendor,
        revision: this.revision
      }).then((res) => {
